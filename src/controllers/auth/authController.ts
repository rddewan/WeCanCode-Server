 ...
 */
const createSendToken = async (user: IUser, statusCode: number, res: Response): Promise<void> => {
    const accessToken = generateAccessToken(user._id);
    const refreshToken = generateRefreshToken(user._id); 
    const expiresIn: number = parseInt(process.env.JWT_COOKIE_EXPIRES_IN || "0", 10);
    const cookieOptions = {
    ...
    await Token.deleteMany({ userId: { $eq: user._id } });
    // hash refresh token
...
export const token = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    // check if refresh token exists
    const { refreshToken } = req.body;
    if (!refreshToken) {
      return next(new AppError('Your refresh token is missing.', 401));
    }
    ...
    let token: IToken = await Token.findOne({ userId: decoded.id }).select('+refreshToken');
    ...
    token = await Token.findOne({ refreshToken: refreshTokenHash }).select('+refreshToken');
    ...
    if (!user || !token) {
      return next(new AppError('User or token does not exist.', 401));
    }
